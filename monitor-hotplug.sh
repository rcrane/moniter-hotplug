...
then
  ...
  elif [ $(xrandr --prop | grep '00ffffffffffff00410c8fc116080000' | wc -l) -eq 1 ] || [ $(xrandr --prop | grep '091b0103803c2278ea3035a7554ea326' | wc -l) -eq 1 ]
  then
  ...
  # If DP1 and internal display are present but DP2 is not

  if [ $(xrandr --prop | grep '00ffffffffffff00410c8fc116080000' | wc -l) -eq 1 ] || [ $(xrandr --prop | grep '091b0103803c2278ea3035a7554ea326' | wc -l) -eq 1 ]
  then
  
    echo $( date +%H:%M:%S ) "--" "Showing desktop on HDMI-1 only" >> /tmp/udev-debug.log
    xrandr --output DP-1 --mode 3840x2160 --primary --output eDP-1 --off
  ...
  # If DP2 and internal display are present but DP1 is not
  ...
  # If HDMI is not connected and internal display is present
  echo $( date +%H:%M:%S ) "--" "Reducing desktop to internal screen" >> /tmp/udev-debug.log
  xrandr --output eDP-1 --auto --primary --output HDMI-1 --off
  
elif [ ! -z "$eDP1" -a -z "$DP1" ]; then

  # If DP1 is not connected and internal display is present
  echo $( date +%H:%M:%S ) "--" "Reducing desktop to internal screen" >> /tmp/udev-debug.log
  xrandr --output eDP-1 --auto --primary --output HDMI-1 --off
...
then
  ...
  killall firefox 
  pgrep -f /usr/lib/firefox | xargs --no-run-if-empty kill
  retVal=$?
  if [ $retVal -eq 0 ]; then
    firefox &
  fi
